...
void StateMachine::setState(States newState) {
  Serial.println("Checking states");
...
void StateMachine::handlePositioningX() {
  ...
  MoveResult moveResult = pusher.moveAbs(60.0);
...
void StateMachine::handleCutting() {
  ...
  if (digitalRead(lidPin)) { // if lid is pressed down, FALSE
